...
struct pipeline_subgroup_info
{
	        ...
	        uint8_t subgroup_maximum_size_log2)
	{
		...
		{
			// Only request varying subgroup size when the device actually has
			// VK_EXT_subgroup_size_control (requiredSubgroupSizeStages != 0). Fixed-size
			// 1.1 GPUs without it (e.g. Tegra X1) run at their natural subgroup size and
			// must not set this flag - it would be invalid without the extension.
			if (prop13.requiredSubgroupSizeStages)
