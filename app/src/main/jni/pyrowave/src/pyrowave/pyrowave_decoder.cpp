...
// --- Robustness workaround: force the 1D uniform-texel-buffer payload path -------------
//
// The decoder reads the entropy payload through one of three paths, chosen at runtime:
//   STORAGE_MODE 0 : 8-bit storage buffer        (desktop AMD/Intel/NVIDIA incl. Tegra X1)
//   STORAGE_MODE 1 : 1D uniform texel buffer     (fallback)
//   STORAGE_MODE 2 : aliased linear 2D textures  (mobile optimisation; Adreno, Mali, ...)
//
// Mode 2 aliases R8/R16/R32 *linear* images over one allocation and addresses them as
// texel (offset % width, offset / width) -- i.e. it ASSUMES the driver lays out each
// linear row with a tight pitch (4096 bytes). The setup code never queries the real row
// pitch (see check_linear_texture_support: "Just assume this works"). Older mobile drivers
// (Adreno 6xx) pad linear rows, so every row past the first is misaligned. Small payloads
// (static frames) sit in the first row and decode perfectly; large payloads (motion) spill
// across rows and turn to garbage -- exactly the reported symptom. The newer Adreno 730
// driver uses a tight pitch and only shows a mild variant of the defect.
//
// Mode 1 is 1D (a plain texel buffer) and has no row pitch, so it is immune to this whole
// class of bug, and it also avoids the mode-0 8-bit path. Forcing it on every device is the
// most-portable configuration. Default ON; set env PYROWAVE_FORCE_TEXEL_BUFFER=0 (or
// -DPYROWAVE_FORCE_TEXEL_BUFFER=0 at build time) to restore the original auto-selection.
// NOTE: tested = NO EFFECT on any device (Adreno 730/640, Tegra X1). Storage path is
// exonerated as the artifact cause; default reverted to 0. Toggle kept for future use.
#ifndef PYROWAVE_FORCE_TEXEL_BUFFER
#define PYROWAVE_FORCE_TEXEL_BUFFER 0
#endif

static bool force_texel_buffer()
{
	static const bool v = [] {
		if (const char * e = std::getenv("PYROWAVE_FORCE_TEXEL_BUFFER"))
			return std::atoi(e) != 0;
		return PYROWAVE_FORCE_TEXEL_BUFFER != 0;
	}();
	return v;
}
...
// VK_KHR_synchronization2 fallback. Old Vulkan-1.1 drivers (e.g. Tegra X1 / 2019-era
// NVIDIA) don't expose vkCmdPipelineBarrier2, so calling it aborts. When the function
// pointer is absent, translate the DependencyInfo's global memory barriers into a legacy
// vkCmdPipelineBarrier. The codec only emits global MemoryBarrier2 entries here (no
// image/buffer barriers) using stage/access bits that exist in the legacy 32-bit enums,
// so truncating the 64-bit masks to 32 bits is exact.
static void pipeline_barrier2_compat(vk::raii::CommandBuffer & cmd, const vk::DependencyInfo & dep)
{
	...
	barriers.reserve(dep.memoryBarrierCount);
	for (uint32_t i = 0; i < dep.memoryBarrierCount; i++)
	{
		const auto & b = dep.pMemoryBarriers[i];
		src_stage |= vk::PipelineStageFlags(uint32_t(static_cast<VkPipelineStageFlags2>(b.srcStageMask)));
		dst_stage |= vk::PipelineStageFlags(uint32_t(static_cast<VkPipelineStageFlags2>(b.dstStageMask)));
		barriers.push_back(vk::MemoryBarrier{
		        .srcAccessMask = vk::AccessFlags(uint32_t(static_cast<VkAccessFlags2>(b.srcAccessMask))),
		        .dstAccessMask = vk::AccessFlags(uint32_t(static_cast<VkAccessFlags2>(b.dstAccessMask))),
		});
	}
	// Legacy pipelineBarrier forbids an empty stage mask.
	if (!src_stage)
		src_stage = vk::PipelineStageFlagBits::eAllCommands;
	if (!dst_stage)
		dst_stage = vk::PipelineStageFlagBits::eAllCommands;
	cmd.pipelineBarrier(src_stage, dst_stage, dep.dependencyFlags, barriers, {}, {});
}
        ...
        decoder(decoder)
{
	...
	try
	{
		// Skip the aliased-linear-2D-image promotion when forcing the texel-buffer path;
		// leaving r8/r16/r32 images null makes decode() select STORAGE_MODE 1 (see header).
		if (!force_texel_buffer())
        ...
        ds_pool(nullptr)
{
	...
	// aggregate zeroed - the subgroup operations are reported in the core 1.1
	// VkPhysicalDeviceSubgroupProperties instead. Backfill from there before the check.
	if (!ops)
	{
	...
	if (prop13.maxSubgroupSize == 0)
	{
		// No VK_EXT_subgroup_size_control (e.g. Tegra X1 / old NVIDIA): the device has a
		// single fixed subgroup size reported by the core 1.1 properties. Use it for
		// min==max and leave requiredSubgroupSizeStages = 0 so set_subgroup_size knows
		// not to request the (unsupported) varying/required-size pipeline flags.
		auto sg = phys_dev.getProperties2<vk::PhysicalDeviceProperties2, vk::PhysicalDeviceSubgroupProperties>();
		uint32_t fixed = sg.get<vk::PhysicalDeviceSubgroupProperties>().subgroupSize;
		prop13.minSubgroupSize = fixed;
		prop13.maxSubgroupSize = fixed;
	}
	{
	...
	if (prop.properties.limits.maxTexelBufferElements >= 16 * 1024 * 1024)
	{
		...
		if (force_texel_buffer() or
		    not feat12.storageBuffer8BitAccess or
	...
	if (force_texel_buffer())
		std::cerr << "PyroWave: forcing STORAGE_MODE 1 (uniform texel buffer)" << std::endl;
	...
	for (size_t i = 0; i < std::size(dequant_); ++i)
	{
		...
		// eRequireFullSubgroups needs computeFullSubgroups (VK_EXT_subgroup_size_control).
		// Fixed-size 1.1 GPUs without it (e.g. Tegra X1) get full subgroups naturally;
		// don't set the flag there (requiredSubgroupSizeStages==0 signals no size-control).
		vk::ComputePipelineCreateInfo info{
		        .stage = {
		                .flags = prop13.requiredSubgroupSizeStages ? vk::PipelineShaderStageCreateFlags{vk::PipelineShaderStageCreateFlagBits::eRequireFullSubgroups} : vk::PipelineShaderStageCreateFlags{},
	...
	for (int input_level = DecompositionLevels - 1; input_level >= 0; input_level--)
	{
		...
		if (output_level == 0 && chroma == ChromaSubsampling::Chroma420)
		{
			...
			if (render_height < extent.height)
			{
				...
				pipeline_barrier2_compat(cmd, dep);
...
bool Decoder::decode(vk::raii::CommandBuffer & cmd, DecoderInput & input, const ViewBuffers & views)
{
	...
	// Upstream 24a8649 ("Fix syncval error"): the post-dequant compute->copy barrier must
	// declare the transfer write it guards, else a WAR/WAW hazard with the next frame's
	// payload upload is missed.
	vk::MemoryBarrier barrier{
	        .srcAccessMask = vk::AccessFlagBits::eNone,
